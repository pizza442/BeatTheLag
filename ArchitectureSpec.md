...
| Method | Parameter | Return | Description|
|--------|-----------|--------|-------------|
|validate()|none|Boolean| Retrieve `userInput` and decide if what the user inserts is relevant or possible to use by using regex expression. If fails, sends an error message through google chrome window. Otherwise call out `organize()`.|
...
Adds a reminder to the event so the user can be notified via a Google notification on their device a few minutes before they should sleep
...
#### Outputs
- `database` - event IDs added to the user’s Google Calendar

### Properties
| Property | Type | Description|
|----------|------|------------|
|IdToken|String|Google authentication token needed to use Google Calendar API for the user|
...
|createEvent()|`sleepingSchedule`|JSON object|creates an event with the date, start time, and end time from the json object parameter.|
|updateEvent()|String|none| Updates the event in the users Google Calendar with the given `eventId`
